...
export async function GET() {
  try {
    const [bills, budget, vehicles, groceries, recipes] = await Promise.all([
      redis.get("bills"),
      redis.get("budget"),
      redis.get("vehicles-list"),
      redis.get("groceries-list"),
      redis.get("recipes-list"),
    ]);
...
export async function POST(request) {
  try {
    ...
    await Promise.all([
      ...
      redis.set("groceries-list", data.groceries || []),
      redis.set("recipes-list", data.recipes || []),
    ]);
